...
export default function FaceMeshOverlay({
  ...
  useEffect(() => {
    ...
    const getRegionColors = (regionName: string, isHovered: boolean) => {
      const score = regions[regionName]?.severity_score ?? 0;
    ...
    Object.entries(REGION_INDICES).forEach(([regionName, indices]) => {
      ...
      const isHovered = activeRegion === regionName;
      ...
      if (isHovered) {
        ...
        const labelText = `${regionName.replace('_', ' ').toUpperCase()} (${Math.round((regions[regionName]?.severity_score ?? 0) * 100)}%)`;
    ...
    Object.entries(regions).forEach(([regionName, analysis]) => {
  ...
  }, [landmarks, regions, width, height, activeRegion]);
